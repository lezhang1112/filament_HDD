...
public:
    ...
    using WebUiCallback = std::function<void(unsigned long long web_hwnd)>;
    ...
    void run(const Config& config, SetupCallback setup, size_t bmpwidth, size_t bmpheight, const char* bmppath,  const char* title_bmppath, CleanupCallback cleanup,
            ImGuiCallback imgui = ImGuiCallback(), WebUiCallback webCallback = WebUiCallback(), EventCallback eventCallback = EventCallback(), ImGuiNotifyCallback imguinotify = ImGuiNotifyCallback(), PreRenderCallback preRender = PreRenderCallback(),
...
private:
    ...
    public:
        ...
        SDL_Window* getWEBWindow() {
            return mWebWindow;
        }
        void setWEBWindow(SDL_Window* webWindow) {
             mWebWindow = webWindow;
        }
    private:
