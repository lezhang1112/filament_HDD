...
ViewerGui::ViewerGui(filament::Engine* engine, filament::Scene* scene, filament::View* view,
    ...
    mLeftSidebarWidth = DEFAULT_LEFT_SIDEBAR_WIDTH;
    mTopMenuHeight = DEFAULT_TOP_MENU_HIEGHT;
...
void ViewerGui::updateRootTransform() {
    ...
    if (mSettings.viewer.autoScaleEnabled) {
        ...
        transform = fitIntoUnitCube(aabb,0);
    }
...
void ViewerGui::renderUserInterface(float timeStepInSeconds, View* guiView, float pixelRatio) {
    ...
    mImGuiHelper->render(timeStepInSeconds, [this](Engine*, View*) {
        ...
        this->updateExternalDataInterface();
        this->updateTopFunctionInterface();
    });
}
...
void ViewerGui::updateTopFunctionInterface(){
    ...
    ImGui::SetNextWindowSize(ImVec2(width, mTopMenuHeight), ImGuiCond_Once);
    ImGui::SetNextWindowSizeConstraints(ImVec2(width, 20), ImVec2(width, height));
    ImGui::Begin("TopFunction", nullptr, ImGuiWindowFlags_NoTitleBar);

    if (mTopMenuUI) {
        mTopMenuUI();
    }
    mTopMenuHeight = ImGui::GetWindowHeight();
    ImGui::End();


}
void ViewerGui::updateExternalDataInterface() {
    using namespace filament;
    ...
    const float width = ImGui::GetIO().DisplaySize.x;
    const float height = ImGui::GetIO().DisplaySize.y;
    ImGui::SetNextWindowPos(ImVec2(0, mTopMenuHeight));
    ImGui::SetNextWindowSize(ImVec2(mLeftSidebarWidth, height - mTopMenuHeight), ImGuiCond_Once);
    ImGui::SetNextWindowSizeConstraints(ImVec2(20, height), ImVec2(width, height));

    ImGui::Begin("ExternalData", nullptr, ImGuiWindowFlags_NoTitleBar);
  
  
    if (mLeftSidebarUI) {
        mLeftSidebarUI();
    }
    mLeftSidebarWidth = ImGui::GetWindowWidth();
    ImGui::End();
...
void ViewerGui::updateUserInterface() {
    ...
    const float width = ImGui::GetIO().DisplaySize.x;
    const float height = ImGui::GetIO().DisplaySize.y;
    ImGui::SetNextWindowPos(ImVec2(width - mSidebarWidth, mTopMenuHeight));
    ImGui::SetNextWindowSize(ImVec2(mSidebarWidth, height - mTopMenuHeight), ImGuiCond_Once);
    ImGui::SetNextWindowSizeConstraints(ImVec2(20, height), ImVec2(width, height));
    ...
    auto acustomLights = utils::FixedCapacityVector<utils::Entity>::with_capacity(mScene->getEntityCount());
    mScene->forEach([&](utils::Entity entity) {
        if (lm.hasComponent(entity)) {
            acustomLights.push_back(entity);
        }
        });

    applySettings(mEngine, mSettings.lighting, mIndirectLight, mSunlight,
        acustomLights.data(), acustomLights.size(), &lm, mScene, mView);
