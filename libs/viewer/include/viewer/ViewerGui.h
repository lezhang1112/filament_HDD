...
public:
     ...
     */
    ~ViewerGui();
    void setCustomAsset(FilamentAsset* asset);
    /**
    ...
    void updateExternalDataInterface();
    void updateTopFunctionInterface();
    /**
    ...
    /**
    * Retrieves the current width of the ImGui "window" which we are using as a leftsidebar.
    * Clients can monitor this value to adjust the size of the view.
    */
    int getLeftSidebarWidth() const { return mLeftSidebarWidth; }

    /**
    * Retrieves the current height of the ImGui "window" which we are using as a topmenu.
    * Clients can monitor this value to adjust the size of the view.
    */
    int getTopMenuHeight() const { return mTopMenuHeight; }
    /**
    ...
    void setLeftSidebarUiCallback(std::function<void()> callback) { mLeftSidebarUI = callback; }
    /**
  * Allows clients to inject custom UI.
  */

    void setTopMenuUiCallback(std::function<void()> callback) { mTopMenuUI = callback; }
    /**
...
private:
    ...
    std::function<void()> mLeftSidebarUI;
    std::function<void()> mTopMenuUI;
